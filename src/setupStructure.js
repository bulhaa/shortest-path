...
export function setupStructure (geojson, edges, points, polygons) {
  ...
  for (let i = 0; i < coords.length; i++) {
    ...
    for (let ii = 0; ii < coords[i].geometry.coordinates.length; ii++) {
      let prevPoint = new Point(coords[i].geometry.coordinates[ii][0], i)
      let currentPoint = new Point(coords[i].geometry.coordinates[ii][1], i)
      prevPoint.nextPoint = currentPoint
      let nextPoint = new Point(coords[i].geometry.coordinates[ii][2], i)
      ...
      for (let iii = 2; iii < coords[i].geometry.coordinates[ii].length - 2; iii++) {
        ...
        nextPoint = new Point(coords[i].geometry.coordinates[ii][iii + 1], i)
